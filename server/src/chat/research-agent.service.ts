...
export class ResearchAgentService {
  ...
  getResearchContextForChat(chatId: string): string | undefined {
    return this.researchContextByChat.get(chatId);
  }
  ...
  startResearchTask(chatId: string, topic: string): StartResearchTaskResponse {
    ...
    const budget: ResearchBudgetProgress = {
      maxRounds: ResearchAgentService.RESEARCH_MAX_ROUNDS,
      currentRound: 0,
      maxDurationMs: ResearchAgentService.RESEARCH_MAX_DURATION_MS,
      elapsedMs: 0,
      etaMs: ResearchAgentService.RESEARCH_MAX_DURATION_MS,
    ...
    const streamSession: ResearchStreamSession = {
      ...
      events: [],
      listeners: new Set(),
      expireAt: now + ResearchAgentService.STREAM_TTL_MS,
      closed: false,
    };
  ...
  getResearchPlan(chatId: string, taskId: string): ResearchPlanResponse {
    ...
    return {
      taskId,
      items: state.plan,
      maxConcurrentBranches: ResearchAgentService.RESEARCH_MAX_CONCURRENT_BRANCHES,
      ...
      while (true) {
        ...
        if (roundExceeded || timeExceeded) {
          if (!state.budget.extensionRoundUsed) {
            ...
            state.budget.maxRounds += ResearchAgentService.RESEARCH_EXTENSION_ROUNDS;
            this.appendResearchEvent(streamSession, {
              eventType: "budget_progress",
              payload: { budget: state.budget },
              done: false,
  ...
  private async processResearchRound(
    ...
    const maxConcurrent = ResearchAgentService.RESEARCH_MAX_CONCURRENT_BRANCHES;
    const startOffset =
  ...
  private async searchWithRetry(
    ...
    for (
      let attempt = 0;
      attempt <= ResearchAgentService.RESEARCH_TAVILY_MAX_RETRIES;
      attempt += 1
    ) {
      try {
        ...
        lastError = error;
        state.failedOrSkippedAttempts += 1;
        if (attempt >= ResearchAgentService.RESEARCH_TAVILY_MAX_RETRIES) {
          break;
        }
        const delay =
          ResearchAgentService.RESEARCH_TAVILY_RETRY_DELAYS_MS[attempt] ??
          ResearchAgentService.RESEARCH_TAVILY_RETRY_DELAYS_MS[
            ResearchAgentService.RESEARCH_TAVILY_RETRY_DELAYS_MS.length - 1
          ] ??
          1000;
  ...
  private isWhitelistDomain(domain: string): boolean {
    ...
    return ResearchAgentService.RESEARCH_WHITELIST_SUFFIXES.some((suffix) =>
  ...
  private touchResearchSession(session: ResearchStreamSession): void {
    session.expireAt = Date.now() + ResearchAgentService.STREAM_TTL_MS;
  }
